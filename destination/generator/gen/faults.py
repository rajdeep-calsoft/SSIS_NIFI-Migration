...
CAUGHT_BY = {
    "missing_field":    "QueryRecord     -> NULL_CRITICAL (field absent)",
    "wrong_type":       "QueryRecord     -> NULL_CRITICAL if it lands on an empty\n"
                        "                     string, else ValidateRecord -> PARSE_ERROR",
    "bad_values":       "QueryRecord     -> RANGE_VIOLATION",
    "unknown_sku":      "LookupRecord    -> UNKNOWN_SKU",
    "unknown_customer": "LookupRecord    -> UNKNOWN_CUSTOMER",
    "late_timestamp":   "QueryRecord     -> BAD_TIMESTAMP (order_ts outside +/-365d)",
    "unicode_currency": "QueryRecord     -> BAD_CURRENCY (not INR/USD/EUR/GBP)",
    "duplicate_id":     "QueryRecord     -> DUP_KEY on (order_id, line_no); the\n"
                        "                     UPSERT still keeps the table correct",
...
def corrupt(record: dict, rng: random.Random, kind: str | None = None) -> tuple[dict, str]:
    ...
    if kind == "missing_field":
        ...
        # "inr" is deliberately NOT here: the flow upper-cases currency before
        # checking it, so a lowercase code is valid data, not a fault. Every
        # value in this list has to be observably wrong.
