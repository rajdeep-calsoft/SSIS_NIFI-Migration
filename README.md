...
3. **Replaces** the Grafana/SQL-view comparison with a report engine that
   discovers each table's primary key and columns at run time via
...
destination/    nifi :8085, postgres :5437 -- the live NiFi the migrated
                                    flow runs on
...
# wait for NiFi's queues to settle (watch http://localhost:8085/nifi)
...
- **No Grafana, no `postgres_fdw`.** The comparison report is a one-shot,
  standalone HTML/JSON file generated by `report/cli.py`, reaching both
  Postgres instances directly over their published ports (the same trick the
  reference repo's own `api/app.py` already uses for its CSV export) — no
  cross-stack Docker networking, no dashboard infrastructure to stand up.
